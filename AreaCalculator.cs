...
namespace AreaCalculatorDesktop
{
    ...
    public class AreaCalculator
    {
        ...
        /// <param name="crossPolygonLines">crossPolygonLines collection</param>
        /// <returns>
        /// Return square by formula squareOverlapLines += A[i] * B[j] * (1 / sin(AngleAB))
        /// </returns>
        ...
        public double SquareLinesInPolygon()
        {
            ...
            return SquareLinesCrossPolygonWithoutOverlap(crossPolygonLines, LinearObjects)
    ...
    public abstract class Polygon
    {
        ...
        {
            _vertices = vertices;
            _edges = CalculateEdges(vertices);
        }
        ...
        /// <returns></returns>        
        protected List<Vector> CalculateEdges(List<Point> vertices)
        {
            ...
            for (int i = 0; i < vertices.Count; i++)
            {
                ...
                {
                    listOfEdges.Add(vertices[i + 1] - vertices[i]);
                }
                else
                {
                    listOfEdges.Add(vertices[0] - vertices[i]);
                }
            }
            return listOfEdges;
        }
    ...
    {
        public Convex(List<Point> vertices) : base(vertices)
        {
            if (!IsConvex())
            {
    ...
    public class LinearEquation
    {
        ...
        /// </summary>
        /// <param name="coefficientA">Сoefficient A</param>
        /// <param name="coefficientB">Сoefficient B</param>
        /// <param name="coefficientC">Сoefficient C</param>
        ...
        public Point CalculatePoint(double coordinate)
        {
            ...
            if ((X == null) && (Y == null))
            {
                ...
                calculatedPoint.Y = (-(CoefficientC / CoefficientB) - (CoefficientA / CoefficientB)
                    * calculatedPoint.X);
            }
            else
            {
    ...
    public static class LowlevelGeometry
    {
        ...
        {
            var collisionPointsLineAndPolygon = new List<Point>();
            for (int i = 0; i < vertices.Count; i++)
            {
                if (i < vertices.Count - 1)
                {
                    ...
                    if (pointCrossingLines != null)
                    {
                        collisionPointsLineAndPolygon.Add((Point)pointCrossingLines);
                    }
                }
                else
                {
            ...
            return collisionPointsLineAndPolygon.Distinct().ToList();
        }
